...
;entradas
#define	    BTN_HEATER 	    PORTB,0
#define	    TROCA_D  	    PORTB,1
#define	    BTN_HIST_ADD    PORTB,2
#define	    BTN_HIST_SUB    PORTB,3  
...
#define	    MUDEI_CANAL	    FLAGS,4
#define	    MUDEI_HEATER    FLAGS,5
...
LACO_PRINCIPAL
    BTFSS   BTN_HEATER	    ;testa se a chave está acionada
    CALL    MUDA_HEATER
    BTFSS   TROCA_D	    ;testa se a chave está acionada
    CALL    TROCA_CANAL
    ...
    BTFSS   QUAL_CANAL	    ;verifica o canal que será lido
...
CANCELA_RUIDO_HEATER
    BTFSS   BTN_HEATER
    MOVWF   FILTRO
    
    DECFSZ  FILTRO,F
    GOTO    CANCELA_RUIDO_HEATER
...
CANCELA_RUIDO_DISPLAY 
    BTFSS   TROCA_D			;Testa se o botão foi pressionado novamente
    MOVWF   FILTRO			;Se ele entendeu que foi pressionado, o filtro reseta
    
    DECFSZ  FILTRO,F			;Filtro--; se Filtro == 0 ele pula a instrução
    GOTO    CANCELA_RUIDO_DISPLAY	;Se não ele vai voltar e continuar o teste
    
    RETURN				;Volta para onde a subrotina foi chamada
    
TROCA_CANAL
    MOVLW   .100
    CALL    CANCELA_RUIDO_DISPLAY
...
TESTA_CENTENA
    ...
    CALL    BUSCA_CODIGO    ;chama a subrotina para buscar o código de 7 segmentos
    ;MOVLW   B'11110110'    ;Escreve H no display
    MOVWF   DISPLAYS	    ;DISPLAYS = W (código de 7 segmentos)
